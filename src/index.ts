...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'ESPN');
}

const BASE = 'https://site.api.espn.com/apis/site/v2/sports';
// Standings live under a different ESPN base (apis/v2, not apis/site/v2).
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    name: 'get_standings',
    description:
      'League standings / table: win-loss records, win %, games behind (and points/draws for soccer), grouped by conference/division. PREFER for "NBA standings", "Premier League table", "NFL division standings", "who is leading <league>". ' +
      PAIRS,
    inputSchema: {
      type: 'object',
      properties: {
        sport: { type: 'string', description: "Sport, e.g. 'basketball', 'soccer'." },
        league: { type: 'string', description: "League slug, e.g. 'nba', 'eng.1'." },
        season: { type: 'number', description: 'Optional 4-digit season year (e.g. 2025). Default = current season.' },
      },
      required: ['sport', 'league'],
    },
  },
  {
    ...
    description:
      "A team's schedule — upcoming and recent games with dates, opponents, and (for finished games) scores and result. PREFER for \"when do the Lakers play next\", \"<team>'s schedule\", \"<team> recent results\". Get the numeric team_id from get_teams. " +
      PAIRS,
    inputSchema: {
      type: 'object',
      properties: {
        sport: { type: 'string', description: "Sport, e.g. 'basketball'." },
        league: { type: 'string', description: "League slug, e.g. 'nba'." },
        team_id: { type: 'string', description: 'Numeric ESPN team id (from get_teams), e.g. "13" for the Lakers.' },
      },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'get_standings': {
      const sport = reqStr(args, 'sport', "'basketball'");
      const league = reqStr(args, 'league', "'nba'");
      const season = typeof args.season === 'number' ? args.season : undefined;
      const qs = season ? `?season=${season}` : '';
      const data = (await espnGet(`/${seg(sport)}/${seg(league)}/standings${qs}`, STANDINGS_BASE)) as StandingsResp;
      if (isError(data)) return data;
      const STAT_KEYS = ['rank', 'wins', 'losses', 'ties', 'points', 'winPercent', 'gamesBehind', 'gamesPlayed', 'pointsFor', 'pointsAgainst'];
      const mapEntries = (entries: StandingsEntry[]) =>
        entries.map((e) => {
          const stats: Record<string, string> = {};
          for (const s of e.stats ?? []) {
            if (s.name && STAT_KEYS.includes(s.name) && s.displayValue != null) stats[s.name] = s.displayValue;
          }
          return { team: e.team?.displayName, abbreviation: e.team?.abbreviation, ...stats };
        });
      ...
      if (groups.length) {
        return {
          ...
          groups: groups.map((g) => ({
            group: g.name ?? null,
            standings: mapEntries(g.standings?.entries ?? []),
          })),
        };
      }
      // Flat (ungrouped) standings fallback.
      return { name: data.name ?? null, standings: mapEntries(data.standings?.entries ?? []) };
    }
    case 'get_team_schedule': {
      const sport = reqStr(args, 'sport', "'basketball'");
      const league = reqStr(args, 'league', "'nba'");
      const teamId = reqStr(args, 'team_id', "'13'");
      const data = (await espnGet(`/${seg(sport)}/${seg(league)}/teams/${seg(teamId)}/schedule`)) as ScheduleResp;
      if (isError(data)) return data;
      const events = data.events ?? [];
      return {
        team: data.team?.displayName ?? null,
        games: events.map((e) => {
          ...
          return {
            ...
            home: home?.team?.abbreviation,
            home_score: scoreOf(home?.score),
            away: away?.team?.abbreviation,
            away_score: scoreOf(away?.score),
          };
        }),
      };
    }
...
async function espnGet(path: string, base: string = BASE): Promise<unknown> {
  ...
  if (!res.ok) {
    ...
    // ESPN sits behind Akamai, which answers a blocked datacenter IP with a 403
    // and an HTML "Access Denied" page. Two things were wrong with returning
    // that verbatim: it booked as class `error` — "Pipeworx has a defect" — for
    // what is an upstream refusing our egress, and it dumped a raw HTML document
    // into the caller's response. Confirmed live 2026-08-24 and failing every
    // day since at least 08-17, so it is a standing block, not a blip.
    if (res.status === 403 && /Access Denied|akamai/i.test(body)) {
      throw new Error(
        `upstream_down: ESPN is refusing requests from this server (HTTP 403). This is a blanket block on ` +
          `the address range the gateway calls from, not a problem with your arguments — the same request ` +
          `works from a residential connection. Nothing to retry; the pack is effectively offline until the ` +
          `block lifts.`,
      );
    }
    if (res.status === 429) throw new Error(`upstream_throttled: ESPN rate-limited this request (HTTP 429).`);
    if (res.status >= 500) throw new Error(`upstream_down: ESPN returned HTTP ${res.status}.`);
    // Keep the status on anything else — an error message that drops the status
    // makes outage-vs-our-bug unanswerable from telemetry alone.
    return { error: res.status, message: body };
  }
  return res.json();
}
...
type Score = string | number | { displayValue?: string; value?: number } | undefined;
function scoreOf(s: Score): string | number | null {
